...
from sgpykit.tools.type_and_property_check_functions import is_sparse_grid
from sgpykit.util import matlab
from sgpykit.util.misc import to_array


def test_is_sparse_grid():
    N = 2
    ...
    dummy_sparse_grid = matlab.struct(knots=None,
                                      size=None,
                                      weights=None,
                                      knots_per_dim=None,
                                      m=None,
    ...
    assert is_sparse_grid(a_tensor_grid) == 0
    assert is_sparse_grid(dummy_sparse_grid) == 1     # struct, so ok
    assert is_sparse_grid(dummy_struct_array_0) == 0  # StructArray, but no entries
    assert is_sparse_grid(dummy_struct_array_1) == 1  # StructArray, 1x1 cells
    assert is_sparse_grid(dummy_struct_array_2) == 1  # StructArray, 1x2 cells
    assert is_sparse_grid(sparse_grid[0]) == 1
    assert is_sparse_grid(sparse_grid) == 1
    assert is_sparse_grid(no_tensor_grid) == 0